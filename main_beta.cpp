...
HRESULT tga_hook(IDirect3DDevice9* dev, unsigned __int8* a2, unsigned int a3, IDirect3DBaseTexture9** a4)
{
    ...
    if (auto *gif = modGifGetDDSByHash(tex->field_60.m_hash, &a3))
        a2 = gif;   // GIF mod: substitute synthesized DDS (D3DX loads DDS from memory)
...
void menu_go_down() {
    ...
    if (current_menu == nullptr || current_menu->used_slots == 0) {
        return;
    }
...
static bool scroll_repeat_should_step(int key_val, int scroll_speed)
{
    ...
    // Effective repeat period: shorter (faster) once held >= 2 s.
    int period = scroll_speed;
    if (key_val >= kScrollAccelHoldFrames) {
        period = (int)(scroll_speed / kScrollAccelFactor);
        if (period < 1) {
            period = 1;
        }
    }
...
void menu_input_handler(int keyboard, int SCROLL_SPEED) {
    if (is_menu_key_clicked(MENU_DOWN, keyboard)) {
        ...
        else if (scroll_repeat_should_step(key_val, SCROLL_SPEED)) {
            menu_go_down();
        }
    }
...
HRESULT __stdcall GetDeviceStateHook(IDirectInputDevice8* self, DWORD cbData, LPVOID lpvData) {
    ...
    if (cbData == 256 || cbData == sizeof(DIJOYSTATE2)) {
        ...
        if (debug_enabled || debug_disabled) {
            ...
            if (!keyboard && controllerKeys[MENU_START] == 2) {
                controllerKeys[MENU_START] = 0;
                toggle = true;
            }
            if (toggle) {
...
void enumerate_mods() {
    ...
    ModGifAnims.clear();
    ModPngTextures.clear();

    const fs::path modsDir = fs::current_path() / "extra";
    ...
    for (const auto& entry : fs::recursive_directory_iterator(modsDir)) {
        ...
        // GIF texture mods (mods/*.gif). Decoded once here; frame 0 is
        // wrapped as a DDS blob consumed by nglLoadTextureTM2 / tga_hook,
        // and animated GIFs are played back by modGifTick() in ngl.cpp.
        // Keyed exactly like .dds mods: hash of the relative path
        // without extension (mods/textures/logo.gif -> "textures/logo").

        tlresource_type resType = TLRESOURCE_TYPE_NONE;
        if (ext == ".gif") {
			resType = TLRESOURCE_TYPE_TEXTURE;
            const fs::path relNoExtGif = fs::relative(path, modsDir).replace_extension();
            const std::string gifKey = transformToLower(relNoExtGif.generic_string());
            modGifRegisterFile(path, to_hash(gifKey.c_str()));
            continue;
        }
        // PNG texture mods (mods/*.png). Decoded once here into a
        // pow2 BGRA image wrapped as a DDS blob that nglLoadTextureTM2
        // consumes exactly like .dds mods. Keyed like .dds/.gif mods:
        // hash of the relative path without extension.
        else if (ext == ".png") {
			resType = TLRESOURCE_TYPE_TEXTURE;
            const fs::path relNoExtPng = fs::relative(path, modsDir).replace_extension();
            const std::string pngKey = transformToLower(relNoExtPng.generic_string());
            modPngRegisterFile(path, to_hash(pngKey.c_str()));
            continue;
        }
        ...
        // Unrecognized extensions stay out of Mods (Type NONE) but remain
        // reachable through ModFileOverrides without bloating memory.
        ...
        // Raw mesh files are requested by bare name ("VENOM" -> VENOM.PCMESH),
        // so nested drops (mods/skins/VENOM.PCMESH) also get keyed by stem.
        if (resType == TLRESOURCE_TYPE_MESH_FILE) {
            ...
            printf("mod: meshfile %s -> 0x%08X (%u bytes)\n",
