...
void enumerate_mods() {
    ...
    for (const auto& entry : fs::recursive_directory_iterator(modsDir)) {
        ...
        // Packer-produced script_executable mash image -> script override,
        // keyed by stem = script name (extra/M01_INTRO.pcsx overrides the
        // "m01_intro" script; a stem in no loaded pack is injected as a
        // brand-new loadable script). Validation + registration:
        // modPCSXRegister (script_object.cpp); served by
