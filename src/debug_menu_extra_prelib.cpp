...
bool sub_6702DE_prelib()
{
    return  MemoryUnitManager::LoadGame(0);
}

int sub_69FD54_prelib(int a1)
{
   MemoryUnitManager::SetCurrentUnit(a1);
}

int sub_B7F990_prelib()
{
  return sub_69FD54_prelib(8);
}

int sub_69A45D_prelib()
{
  return sub_B7F990_prelib();
}


void sub_1059CD0_prelib(int a1)
{
    dword_151691C() = a1;
}
...
int sub_B7F9D0_prelib()
{
    ...
    while (sub_6702DE_prelib()) {
    }

    return 0;
}

int sub_66FBE0_prelib(void)
{
  return sub_B7F9D0_prelib();
}

void district_variants_handler_prelib(debug_menu_entry *entry)
{
...
std::string district_variants_render_callback_prelib(debug_menu_entry *a2)
{
...
void populate_district_variants_menu_prelib(debug_menu_entry *entry)
{
    ...
    if ( the_terrain != nullptr )
    {
        ...
        for ( auto idx = 0; idx < num_regions; ++idx )
        {
            ...
            if ( district_variant_count > 1 )
            {
                ...
                v11.set_id(idx);
                v11.set_render_cb(district_variants_render_callback_prelib);
                v11.set_ival(district_variant);
                v11.set_min_value(0.0);
                v11.set_max_value((float)(district_variant_count - 1));
                v11.set_game_flags_handler(district_variants_handler_prelib);
                v5->add_entry(&v11);
            }
        }
    }
...
void create_debug_district_variants_menu_prelib(debug_menu *parent)
{
    debug_menu_entry v5 {mString {"District variants"}};

    v5.set_submenu(nullptr);
    v5.set_game_flags_handler(populate_district_variants_menu_prelib);
    parent->add_entry(&v5);
}

void populate_gamefile_menu_prelib(debug_menu_entry *entry)
{
    auto& v1 = entry->text;
...
void create_gamefile_menu_prelib(debug_menu* parent)
{
    auto* game_file_menu = create_menu("Saved Game Settings");
    auto* v2 = create_menu_entry(game_file_menu);
    v2->set_game_flags_handler(populate_gamefile_menu_prelib);
    parent->add_entry(v2);
}

void warp_handler_prelib(debug_menu_entry *entry)
{
    auto idx = entry->get_id();
    auto *the_terrain = g_world_ptr->the_terrain;
    if ( the_terrain != nullptr )
...
void warp_poi_handler_prelib([[maybe_unused]] debug_menu_entry *menu_entry)
{
...
void populate_warp_menu_prelib(debug_menu_entry *entry)
{
    ...
    v19.set_game_flags_handler(warp_poi_handler_prelib);
    v20->add_entry(&v19);
    auto *the_terrain = g_world_ptr->the_terrain;
    if ( the_terrain != nullptr )
    {
        ...
        for ( auto idx = 0; idx < num_regions; ++idx )
        {
            ...
            if ( (reg->flags & 0x4000) != 0 )
            {
                ...
                v15.set_id(idx);
                v15.set_game_flags_handler(warp_handler_prelib);
                v20->add_entry(&v15);
            }
        }
    }
...
void create_warp_menu_prelib(debug_menu *parent)
{
    debug_menu_entry v5 {mString {"Warp"}};

    v5.set_submenu(nullptr);
    v5.set_game_flags_handler(populate_warp_menu_prelib);
    parent->add_entry(&v5);
}

inline std::string ngl_render_callback_prelib(debug_menu_entry *a2)
{
...
inline void ngl_handler_prelib(debug_menu_entry *a1)
{
    auto v3 = a1->get_ival();
...
void create_ngl_menu_prelib(debug_menu *parent)
{
...
void create_debug_render_menu_prelib(debug_menu *parent)
{
	...
	create_ngl_menu_prelib(debug_render_menu);
...
std::string camera_render_callback_prelib(debug_menu_entry *a2)
{
    if ( a2 == nullptr ) {
...
void game_flags_handler_prelib(debug_menu_entry *a1)
{
    enum {
        SLOW_MOTION = 2u,
    };
    ...
    case 9u:
    {
        ...
        auto *v23 = static_cast<input_device *>(v6->field_30[1]);
 
        // Controller 2 needs a live device object AND a physical connection
        // before we hand the user-cam over to it. Vtable offset +44 is
        // input_device::is_connected, +8 is input_device::get_id (see
        // input_device.h). A null slot pointer is folded into the
        // "not connected" path to avoid dereferencing an empty slot on
        // builds where joypad index 1 was never populated.
        if ( v23 == nullptr || !v23->is_connected() )
        {
            debug_print_va("Controller 2 is not connected!");
            a1->set_bval(false, true);
            v24 = false;
        }
 
        if ( v24 )
        {
            debug_print_va("User cam (theoretically) enabled on controller 2");
            // mouselook_controller::field_14 is the device id the camera
            // reads from (seeded from input_mgr's current device in the
            // ctor). Point it at controller 2's device id.
            g_mouselook_controller()->field_14 = v23->get_id();
        }
        else
        {
            g_mouselook_controller()->field_14 = INVALID_DEVICE_ID;
        }
    ...
    {
        // Enabled  -> R3 up tilts the camera down, R3 down tilts it up.
        // Disabled -> normal look.
        // camera_mode::update() reads gamefile->field_340.m_invert_camera_vert and
        // folds it into the vertical look-around as v37 = (invert ? -1.0f : 1.0f),
        // so flipping this flag is all that is needed. Horizontal axis is left alone.
        g_game_ptr->get_game_settings()->field_340.m_invert_camera_vert = a1->get_bval();
        break;
    }
...
void create_camera_menu_items_prelib(debug_menu *parent)
{
    assert(parent != nullptr);

    auto *new_entry = create_menu_entry(mString {"Camera"});

    new_entry->set_render_cb(camera_render_callback_prelib);
    const float v1[4] {0, 2, 1, 1};
    new_entry->set_fl_values(v1);
    new_entry->set_game_flags_handler(game_flags_handler_prelib);
    new_entry->set_id(18);
